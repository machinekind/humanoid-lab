...
The consequence: anything that compares the two sides has to compare
DISPLACEMENTS, not positions. An absolute left-right residual on this model
bottoms out at that 1.0e-4 m floor, never at zero, so a position-based
comparison cannot tell a real mismatch from the export's own bias.
