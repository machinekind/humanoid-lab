...
def test_load_robot_spec_parses_toy_fixture():
    ...
    assert spec.group_of("left_hip_pitch_joint") == "hip_pitch"
    assert spec.group_of("right_knee_joint") == "knee"
    ...
    assert spec.foot_sites == ["left_foot", "right_foot"]
    assert spec.foot_geoms == ["left_foot_geom", "right_foot_geom"]
    ...
    assert kf.base_pos == (0.0, 0.0, 0.7074)
...
def test_validate_against_model_catches_actuated_passive_overlap():
    ...
    bogus_spec = dataclasses.replace(
        ...
        passive_joints={
            **spec.passive_joints,
            "left_hip_pitch_joint": spec.passive_joints["toe_joint_a"],
        },
    )
    model = _toy_model()
