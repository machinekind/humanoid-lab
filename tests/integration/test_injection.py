...
def test_build_spec_pd_compiles_and_matches_preset():
    ...
    assert actuator_names == ["hip_pitch_joint", "knee_joint"]  # canonical actuated_joints order

    hip = model.actuator("hip_pitch_joint")
    ...
    knee = model.actuator("knee_joint")
    ...
    assert model.joint("hip_pitch_joint").armature[0] == pytest.approx(0.05)
    assert model.joint("knee_joint").armature[0] == pytest.approx(0.04)
    assert model.joint("hip_pitch_joint").frictionloss[0] == pytest.approx(0.1)
    # The preset omits knee frictionloss, so the XML's own 0.25 must survive.
    assert model.joint("knee_joint").frictionloss[0] == pytest.approx(0.25)
    ...
    assert list(key.qpos[base_addr : base_addr + 3]) == pytest.approx([0.0, 0.0, 0.8])
    assert list(key.qpos[base_addr + 3 : base_addr + 7]) == pytest.approx([1.0, 0.0, 0.0, 0.0])
    hip_addr = model.joint("hip_pitch_joint").qposadr[0]
    knee_addr = model.joint("knee_joint").qposadr[0]
    assert key.qpos[hip_addr] == pytest.approx(0.2)
    assert key.qpos[knee_addr] == pytest.approx(-0.4)
...
def test_pd_force_matches_position_servo_law():
    ...
    hip_qadr = model.joint("hip_pitch_joint").qposadr[0]
    hip_vadr = model.joint("hip_pitch_joint").dofadr[0]
    data.qpos[hip_qadr] = 0.3
    data.qvel[hip_vadr] = -0.5
    data.ctrl[0] = 1.0  # hip actuator is index 0 (canonical order)
...
def test_resolve_carries_velocity_limit():
    ...
    assert params["hip_pitch_joint"].velocity_limit == pytest.approx(10.0)
    assert params["knee_joint"].velocity_limit == pytest.approx(12.0)
...
def test_action_scale_pd():
    ...
    assert scales["hip_pitch_joint"] == pytest.approx(0.3 * 40.0 / 50.0)
    assert scales["knee_joint"] == pytest.approx(0.3 * 55.0 / 60.0)
...
def test_action_scale_ideal_torque():
    ...
    assert scales["hip_pitch_joint"] == pytest.approx(40.0)
    assert scales["knee_joint"] == pytest.approx(55.0)
