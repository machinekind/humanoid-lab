...
WORK=/workspace/humanoid-lab
```

For this repository: module `humanoid_lab`, launcher `./run.sh`, safe process pattern
`humanoid_lab[.]train`.
...
sshc "cd '$WORK' && uv venv --python 3.12 .venv && uv pip install -e ".[dev]""
...
sshc "cd '$WORK' && rm -f run_NAME.log && (PYTHONUNBUFFERED=1 nohup ./run.sh train \
  task=joystick run_name=NAME > run_NAME.log 2>&1 < /dev/null &) && echo LAUNCHED"
```
...
skills/vastai-gpu-training-ops/scripts/watch_run.sh \
  ...
  -p "humanoid_lab[.]train" -m ./run_NAME.mirror.log
```
...
`training-mjx-locomotion` diagnostics.

## Sync after every stage

```bash
rsync -az -e "ssh -p PORT" root@HOST:"$WORK/runs/NAME/" ./runs/NAME/
scp -P PORT root@HOST:"$WORK/run_NAME.log" root@HOST:"$WORK/run_NAME.resolved.yaml" \
  root@HOST:"$WORK/*.mp4" ./runs/NAME/
```

Remote `run.json` may contain absolute checkpoint paths — normalize after syncing if the local
loader does not resolve the run directory. `runs/` is gitignored: a local rsync is not
